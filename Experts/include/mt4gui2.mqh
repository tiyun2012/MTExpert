...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Compatible with dll version 2.6.*
#import "mt4gui2.dll"
	//~~~~~~ Diverse
   string guiVersion			();   
...
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
bool _CloseByTicket(int __SLIPPAGE,int Ticket)
{	
	if (OrderSelect(Ticket, SELECT_BY_TICKET)) 
	{			 
	  RefreshRates();
	  if (OrderType()==OP_BUY) return(OrderClose(OrderTicket(),OrderLots(),MarketInfo(OrderSymbol(),MODE_BID),__SLIPPAGE));
	  if (OrderType()==OP_SELL) return(OrderClose(OrderTicket(),OrderLots(),MarketInfo(OrderSymbol(),MODE_ASK),__SLIPPAGE));
...
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=   
void ButtonBool(int __ihwnd,int button,bool state,color truecolor,color falsecolor, string truetxt="",string falsetxt="")
{
if (button>1)
{
	
   if (state) guiSetBgColor(__ihwnd,button,truecolor);
   if (!state) guiSetBgColor(__ihwnd,button,falsecolor);
   if (truetxt!="" && state) guiSetText(__ihwnd,button,truetxt,15,"Times New Roman");
   if (falsetxt!="" && !state) guiSetText(__ihwnd,button,falsetxt,15,"Times New Roman");         
}
}

//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// adds a button to chart
int addButton(int __ihwnd,int x,int y,int w,int h,string txt,int bgcolor,int txtcolor)
   {   
   int myb = guiAdd(__ihwnd,"button",x,y,w,h,txt); guiSetBgColor(__ihwnd,myb,bgcolor); guiSetTextColor(__ihwnd,myb,txtcolor);
   return (myb);
   }
